...
Training loops for all three phases described in the paper:
    ...
    Phase 3 — Joint Fusion   : End-to-end tsFormer training (Section 3.3)
...
def _build_optimizer_and_scheduler(
    ...
    decay_params = [
        p for n, p in model.named_parameters() if p.requires_grad and p.ndim >= 2
    ]
    no_decay = [
        p for n, p in model.named_parameters() if p.requires_grad and p.ndim < 2
    ]
    ...
    cosine = CosineAnnealingLR(
        opt, T_max=max(1, n_steps - warmup_steps), eta_min=lr * 0.1
    )
...
class EmbeddingClassifier(nn.Module):
    ...
    def __init__(
        ...
        from tsformer.model import ClassificationHead

        self.transformer = transformer

        self.input_dim = head_in if head_in is not None else (d_model * 2)
        self.head = ClassificationHead(self.input_dim, hidden_dims, dropout)
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # If the model was loaded with a larger head (e.g., 192)
        # but we are in Late Fusion mode (emb = 128), we perform automatic padding.
        if emb.size(-1) < self.input_dim:
            padding = torch.zeros(
                (emb.size(0), self.input_dim - emb.size(-1)),
                device=emb.device,
                dtype=emb.dtype,
            )
...
# ──────────────────────────────────────────────────────────────────────────────
# Phase 3 — Joint Fusion (tsFormer)  (Section 3.3)
# ──────────────────────────────────────────────────────────────────────────────


class JointFusionTrainer(FineTuneTrainer):
    """
    Trains the full tsFormer model end-to-end:
    ...
    def fit(
        ...
        print(f"\n{'=' * 60}")
        print("  Phase 3 — Joint Fusion (tsFormer)")
