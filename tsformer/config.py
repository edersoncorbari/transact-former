...
Usage
─────
    from tsformer.config import tsFormerConfig, TrainConfig, load_config, save_config

    cfg = tsFormerConfig()          # sensible defaults
...
class JointFusionConfig:
    """Section 3.3 — tsFormer joint fusion training."""
...
class tsFormerConfig:
    ...
    @classmethod
    def for_paper_24m(cls) -> "tsFormerConfig":
        cfg = cls()
    ...
    @classmethod
    def for_paper_330m(cls) -> "tsFormerConfig":
        cfg = cls()
    ...
    @classmethod
    def for_local_test(cls) -> "tsFormerConfig":
        cfg = cls()
        cfg.model.transformer = TransformerConfig.small()

        # cfg.train.finetune.head.hidden_dims = [64]
        # cfg.train.joint_fusion.head.hidden_dims = [64]
        cfg.model.head.hidden_dims = (64,)
...
def save_config(cfg: tsFormerConfig, path: str | Path) -> None:
    path = Path(path)
...
def load_config(path: str | Path) -> tsFormerConfig:
    ...
    return _build(tsFormerConfig, d)
